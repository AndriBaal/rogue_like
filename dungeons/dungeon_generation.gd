...
class Dungeon:
	...
	func _init(options: DungeonOptions) -> void:
		...
		self.rooms = [{"room": starting_room, "children": []}]
	...
	func _grow_room(room):
		...
		var room_sum = (
			self.good_rooms_left
			+ self.bad_rooms_left
			+ self.neutral_rooms_left
			+ self.boss_rooms_left
		)
		...
		for entrance in entrances:
			if self.rng.randi_range(0, 1) > 0:
				for tile in [entrance["start"], entrance["end"]]:
					...
					for i in range(5):
						tilemap.set_cell(
							tile + i * entrance.direction,
							FLOOR_TILES,
							floor_tile_source.get_tile_id(floor_tile)
						)
					...
					if entrance_layer_data:
						...
						if transpose:
							...
							if flip_h and flip_v:
								...
								direction = Vector2i(0, 1)
						entrances.push_back(
							{"start": coords, "end": coords + neighbour, "direction": direction}
						)
						break
